...
pred.rf <- predict(rf.modelProba, test, type = "prob")
pred.rf

#Ensemble
pred.ens <- (pred.xgb+pred.svm[,1]+pred.rf[,2])/3
pred.ens

#Submission
submission <- cbind('MoleculeId'=1:length(pred.ens), "PredictedProbability"=pred.ens)
write.csv(submission, file="submissionEnsembleSVM_XGB_RF.csv" , row.names = F)
