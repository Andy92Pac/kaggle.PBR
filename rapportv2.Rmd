...
Nous vérifions ensuite que chacune des colonnes du dataset possède au moins deux valeurs distinctes.

```{r}
#Compte le nombre de valeur uniques pour chaque colonne
x <- apply(train.data, 2, function(z) length(unique(z)))
#Vérification qu'aucune colonne n'a qu'une seule valeur
x[x==1]
```

Nous nous assurons par la suite que le jeu de données ne contient pas de valeurs manquantes.

```{r}
na.value <- sum(is.na(train.data))
na.value
```
...
SVM (Support Machine Vectors) est une méthode de classification binaire. Le principe est de rechercher une règle de décision basée sur une séparation par hyperplan de marge optimale entre 2 classes.
...
Le stacking (ou dit parfois blending) est un procédé qui consiste à utiliser deux ou plusieurs classifiers de récupérer leurs sorties pour l'appliquer à un ou des autres classifieurs. Tout cela dans le but d'avoir une meilleure précision donc un modèle plus performant. 
 
D’une certaine façon, il s’agit de prédire quels sont les meilleurs classifieurs et de les pondérer. Cette démarche a l’avantage de pouvoir agréger des modèles très différents et d’améliorer sensiblement la qualité de la prédiction finale, le challenge NetFlix à 1 million $ en est la meilleure preuve.(Méthode d'ensembling stacking utilisée)
...
#Prédiction réalisée sur l'ensemble du jeu de données Test
...
Attribution des poids et Ensembling : Nous n'avons pas appliqué de fonction nous permettant de trouver un poids optimal nous y sommes allé au tatonement et nous avons remarqué pour ces poids utilisés nous obtennons les meilleures résultats de prédiction

```{r eval=FALSE}
#Poids attribué à la prédiction de xgboost
w.xgb = 5

#Poids attribué à la prédiction de SVM
w.svm = 3

#Calcul de l'ensemble des 2 prédictions
...
Nous pouvons désormais procéder à l'évaluation de nos modèles et comparer les résultats obtenus.

```{r}
df = data.frame(Modele = c('SVM', 'XGboost', 'Stacking XGboost RandomForest','Stacking SVM XGboost', 'Stacking Weighted SVM XGboost','Stacking SVM XGboost stretched'), LogLoss.kaggle = c(0.46203,0.45047,0.42802,0.42371,0.42204,0.42065))
df
```

Le meilleur des modèles utilise deux méthodes d'ensembling, le boosting avec l'algorithme xgboost et le stacking en associant ce premier modèle à un modèle créé avec la méthode svm. En effet, l'association de ces deux modèles nous permet d'améliorer les résultats de manière significative.
...
Nous avons répondu à la problématique de ce sujet qui était de construire un modèle capable de faire des prédictions sur l'appartenance d'une molécule à une classe donnée en fonction de ses propriétés chimiques.

Par une méthode d'ensembling qui est le Stacking nous y avons répondu et au final avons eu les score suivants : 

Score public : 0.42065 et Score privée : 0.38882

Les résultats que nous sommes parvenus à obtenir nous semblent plutot satisfaisants mais nous sommes conscient qu'il est toujours possible de parfaire ces derniers. Pour cela nous avons envisager quelques pistes d'améliorations parmi lesquelles :
    + La réduction des dimensions par une approche de régression linéaire logistique binomiale (backward, forward).
    + L'utilisation d'un modèle prédictif au lieu de la moyenne lors de l'application du stacking sur des modèles.
    + L'utilisation du GridSearch sur la totalité des modèles créés et pourquoi pas l'ajout d'un RandomSearch.
